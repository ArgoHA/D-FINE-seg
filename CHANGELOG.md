...
- New `train.in_channels` config (default 3). Set to N>3 to train on stacked modalities (RGB + thermal, RGB + depth, multispectral, etc.).
...
- HGNetv2 stem conv is rewired for arbitrary `in_channels`. Pretrained 3-channel weights are reused: stem is inflated to N channels by tiling the RGB filter mean, so COCO-pretrained fine-tuning still works out of the box.
