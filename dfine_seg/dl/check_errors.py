...
from dfine_seg.dl.validator import match_instances
...
def save_case(
    ...
    cv2.imwrite(str(out_img_dir / f"{Path(img_path).stem}_{case_id}.jpg"), draw)
...
def check_results(
    ...
    """Save false positives and negatives using Validator's matching policy."""
    ...
    if len(preds) == 0:
        ...
        pred_boxes_xyxy_abs = torch.as_tensor(preds["boxes"], dtype=torch.float32)
        pred_labels = torch.as_tensor(preds["labels"], dtype=torch.long)
        pred_scores = torch.as_tensor(preds["scores"], dtype=torch.float32)

    target_boxes = torch.as_tensor(targets["boxes"], dtype=torch.float32).reshape(-1, 4)
    target_boxes = torch.as_tensor(
        norm_xywh_to_abs_xyxy(target_boxes.numpy(), height, width), dtype=torch.float32
    )
    target_labels = torch.as_tensor(targets["labels"], dtype=torch.long)
    ...
    ious = box_iou(pred_boxes_xyxy_abs, target_boxes)
    matches, false_positives, false_negatives = match_instances(
        ious, pred_labels, target_labels, iou_thresh
    )
    for pred_idx, target_idx, _ in matches:
        if pred_labels[pred_idx] != target_labels[target_idx]:
            false_positives.append(pred_idx)
            false_negatives.append(target_idx)

    for pred_idx in sorted(false_positives):
        save_case(
            "FPs",
            img,
            pred_boxes_xyxy_abs[pred_idx],
    ...
    for target_idx in sorted(false_negatives):
        save_case(
            "FNs",
            img,
            target_boxes[target_idx],
            int(target_labels[target_idx]),
            0,
            img_path,
            output_dir,
            label_to_name,
